...
# def one_processing(read_url,write_url):
    ...
    """补全数据,归一化字符数据"""
    data = pd.read_csv(read_url, sep=None, dtype=np.str, header=None, engine='python', chunksize=10000)
    ...
    for j, o1 in enumerate(data):
        ...
        # print()
        count = 0
        for i in o1.index:#i is row index,col is columns index
            # count = 1
            one_row = [scale_a[i, 0]]
            for col in range(1,4,1):#added other data like DLC content

                if col == 2:#DLC DLC max =8
                    a = float(o1.at[i,col]) / 8
                    one_row.append(a)

                else:
                    for elem in o1.at[i,col]:#max = 15
                        try:
                            a = float(elem) / 15
                            # one_row.append()
                        except ValueError:
        ...
        bb.to_csv(write_url,sep=' ', index=False, header=False, mode='a', float_format='%.3f')
        # # np.savetxt(write_url,bb,fmt='%.4f',delimiter='\t',newline='\n',)
        # np.loadtxt(write_url,bb)
        if j %10==0:
            print("current:{},at:{}".format(name,j))
...
if __name__ == "__main__":
    ...
    source_url = []
    dire_url = []
    had_done = ['Attack_free_dataset2.txt']
    ...
    pool.map(one_processing, zip(source_url, dire_url),)
    pool.close()
    pool.join()
