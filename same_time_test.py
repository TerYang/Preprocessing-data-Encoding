...
if __name__ == "__main__":
    ...
    # # raw_to_hackingdata((source_urls[0],dire_urls[0]))
    # for source in raws:
    #     des = os.path.join(dest, 'sameID',os.path.splitext(os.path.basename(source))[0] + same+'.pkl')
    #     print(des)
    #     data = pd.read_pickle(source,compression='zip')
    #     # data = pd.read_csv(source_addr,sep=None,delimiter=' ',dtype=np.str,header=None,engine='python',encoding='utf-8')
    #     filename = os.path.splitext(os.path.basename(source))[0]
    #     # print(data.loc[:10,:])
    #     # res = Mutual_time_interval(data.loc[:, 0].values,'ignore')
    #     # cal ignore id interval time
    #
    ...
    #     res.to_csv(url,sep=' ',index=None,columns=None,header=None,encoding='utf-8',mode='w',float_format='%.6f')
    #     print('*'*60,end='\n\n')
        # exit()
    ...
    # dest = '/home/gjj/dataset/hacking/ignore_ID_diff_time'
    # t = '/home/gjj/dataset/hacking/Intervaltime/ignoreID'
    # # dest = '/home/gjj/dataset/ignore_ID_diff_time'
    # #
    # ignore = '_ignore_id_time_interval.pkl'
    # same = '_same_id_time_interval.pkl'
    # #
    ...
    # heads = [os.path.join(t,os.path.splitext(f)[0] + ignore) for f in os.listdir(source) if '.pkl' in f]
    ...
    #     # print(head.loc[:10,0],end='\n\n')
    #
    #     print('body url:{},shape:{}'.format(body_,body.shape))
    #     # print(body.loc[:10,:],end='\n\n')
    #     data = pd.concat([head, body.loc[:,1:]],axis=1).astype(np.float64)
    #     data.to_pickle(des_,compression='zip')
    #     print('dest url:{},shape:{}'.format(des_,data.shape))
    #     print(data.loc[:10,:],end='\n\n')

        # exit()
        # data.to_csv(des,sep=' ',index=False, header=False, mode='w',encoding='utf-8',float_format='%.6f')#,float_format='%.8f'
    #     # break

    """scalar to -1,1, all saclar or batch size scalar"""
    # source = '/home/gjj/dataset/same_ID_diff_time'
    ...
    # # print(destes)
    # f = lambda x,y:x if operator.ge(x,y) else y
    # for raw,des in list(zip(raws,destes)):
    #     data = pd.read_pickle(raw, compression='zip')
    #     datashape = data.shape
    #     filename = os.path.splitext(os.path.basename(raw))[0]
    #     print(filename, datashape)
    ...
    #     row_ceil = math.ceil(rows / size)
    #     row_floor = math.floor(rows / size)
    #     print('data rows ceil | floor:{}|{}'.format( row_ceil, row_floor))
    #     c_max = [0 for _ in range(columns)]
    #     count = 0
    #     for start in range(row_ceil):
    #         # print(start)
    ...
    #             # block.set_index(keys=, drop=True)
    #         else:
    #             scale_a = scaler.fit_transform(
    #                 data.loc[start * size:start * size + size - 1, :].values.astype(np.float64)).reshape((-1, columns))
    #             # print(data.loc[start * size:start * size + size - 1, :])
    #         c_max = list(map(f, c_max, scaler.data_max_))
    #         # print('scale_a.shape:{}'.format(scale_a.shape))
    #         # print(scale_a)
    #         count += scale_a.shape[0]
    #         if start % 1000 == 0:
    ...
    #         # scale_a = np.around(scale_a, decimals=3)
    #
    #         pd.DataFrame(scale_a).to_csv(des, sep=' ', index=False, header=False, mode='a', encoding='utf-8', float_format='%.4f',
    #                        index_label=None)
    ...
    #     print(des)
    #     data = pd.read_csv(raw,sep=None,delimiter=' ',dtype=np.str,header=None,engine='python',encoding='utf-8')#,nrows=100)
    #     # print(data.loc[:10,:])
    #     # continue
    #     data.to_pickle(des, compression='zip')
    #     print(data.shape)
